...
const primitiveFunctions = {
    getTextFromInput: {
        ...
        function: (input) => {
          ...
          if (input && typeof input.text === 'string') return input.text; // Return the text if input is {text: "..."}
          if (typeof input === 'string') return input; // Return the text if input is already a string
    ...
    // 이곳에 11.md에 있던 나머지 모든 함수들을 포함합니다.
    // (예: exportToDocumentSystem, trimWhitespace, normalizeNewlines 등)
};

module.exports = { primitiveFunctions };
